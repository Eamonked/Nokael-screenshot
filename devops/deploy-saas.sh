...
# Usage: ./deploy-saas.sh [environment]
...
log "Starting SaaS deployment for environment: $ENVIRONMENT"
...
if [ ! -f "$BACKEND_DIR/.env" ]; then
    ...
    if [ -f "$BACKEND_DIR/env.saas.example" ]; then
        cp "$BACKEND_DIR/env.saas.example" "$BACKEND_DIR/.env"
...
if ! grep -q "DEPLOYMENT_MODE=saas" "$BACKEND_DIR/.env"; then
    error "Environment file must have DEPLOYMENT_MODE=saas for SaaS deployment"
    exit 1
fi
...
log "SaaS deployment completed successfully!"
echo ""
